...
function getResponseDef(responses, schemaStyle, localize, includeExample = false) {
  ...
  for (const statusCode in responses) {
    ...
    for (const contentType in responses[statusCode].content) {
      const example = _.get(responses[statusCode].content, 'application/json.schema.example');
      const responseDef = [
        { text: `${_.capitalize(localize.responseModel)} - ${contentType}`, margin: [0, 10, 0, 0], style: ['small', 'b'] },
      ];
      if (example) {
        const formatter = JSON.stringify(example, null, '\u200B\t\u200B\t');
        responseDef.push(
          { text: 'Example', margin: [0, 10, 0, 12], style: ['small', 'b'] },
        );
        responseDef.push(
          {
            table: {
              ...
              body: [
                [{
                  text: formatter, fillColor: '#333333', color: 'white', style: ['small'],
                }],
              ],
            },
            layout: 'noBorders',
